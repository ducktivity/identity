// The Ducktivity identity service: the single source of truth for accounts and
// the SOLE issuer of session tokens for the whole suite. It owns the users table,
// passwordless email-OTP login, the suite-wide entitlement (billing), and EdDSA
// token signing; it publishes its public keys at /.well-known/jwks.json so every
// app backend can verify tokens without holding any signing key (see
// platform-go/authverify).
module github.com/ducktivity/identity/backend
...
require (
	github.com/ducktivity/platform-go v0.1.0
