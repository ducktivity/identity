...
// AuthRequest sends a 6-digit login code, creating the account if needed. It always
// returns a generic acknowledgement so it cannot probe which emails have accounts.
...
// AuthVerify exchanges a valid email + code for a session token. The token carries
// the account's current suite-wide entitlement, so every app learns paid access
...
func AuthVerify(w http.ResponseWriter, r *http.Request) {
	...
	// A missing user is reported with the same generic 401 as a wrong code so the
	// endpoint can't enumerate accounts.
